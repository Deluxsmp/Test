                ...
                if (qIndex !== -1) {
                    ...
                    try {
                        ...
                        if (queueMsg) {
                            ...
                            // পুরনো এমবেড থেকে হোস্ট বা টেস্টিং তথ্য রিড বা রিটেন করার জন্য
                            const oldEmbed = queueMsg.embeds[0];
                            let hostField = '';
                            if (oldEmbed.description && oldEmbed.description.includes('👑 **Queue Host:**')) {
                                const parts = oldEmbed.description.split('👑 **Queue Host:**');
                                if (parts[1]) hostField = '\n\n👑 **Queue Host:**' + parts[1];
                            }

                            const updatedEmbed = new EmbedBuilder()
                                .setTitle(`🎮 Queue: ${gameMode.toUpperCase()}`)
                                .setDescription(desc + hostField)
                                .setColor('Blue');
...
client.on('interactionCreate', async interaction => {
    ...
    // রেজাল্ট ফর্ম বাটন ক্লিক করলে মোডাল ওপেন হবে (শুধুমাত্র টেস্টার এবং এডমিনদের জন্য)
    if (interaction.isButton() && interaction.customId.startsWith('open_result_form_')) {
        const isTester = interaction.member.roles.cache.has(TESTER_ROLE_ID);
        const isAdmin = interaction.member.permissions.has(PermissionsBitField.Flags.Administrator);

        if (!isTester && !isAdmin) {
            return interaction.reply({ 
                content: '❌ Only testers can use this result form!', 
                ephemeral: true 
            });
        }
    ...
    // রেজাল্ট মোডাল সাবমিট হওয়ার পর ডেটাবেজ আপডেট, চ্যানেলে পাঠানো, টিকিট ক্লোজ ও কিউ থেকে রিমুভ করা
    ...
    if (interaction.isButton() && (interaction.customId.startsWith('join_queue_') || interaction.customId.startsWith('leave_queue_'))) {
        ...
        db.get(`SELECT * FROM players WHERE discord_id = ?`, [userId], async (err, playerRow) => {
            ...
            // কিউ আপডেট হওয়ার সময়ও যাতে হোস্টের মেনশন ঠিকভাবে বজায় থাকে
            const oldEmbed = interaction.message.embeds[0];
            let hostField = '';
            if (oldEmbed && oldEmbed.description && oldEmbed.description.includes('👑 **Queue Host:**')) {
                const parts = oldEmbed.description.split('👑 **Queue Host:**');
                if (parts[1]) hostField = '\n\n👑 **Queue Host:**' + parts[1];
            }
